...
%%
function [P] = VCLFD_Python (fin)
  
  if isempty(fin)
    disp('Error! No lightfield is given');
    return;
  end
  ...
  LF = loadLF_Python( fin, param.uCamMovingRight, param.vCamMovingRight, 'lab');
  ...
  LFrgb = single(loadLF_Python( fin, param.uCamMovingRight, param.vCamMovingRight, 'rgb')) ./ 255;
  LFrgb = num2cell(cat(4, squeeze(LFrgb(:, :, :, :, param.cviewIdx)), squeeze(LFrgb(:, :, :, param.cviewIdx, :))), [1 2 3]);

  LFg = single(loadLF_Python( fin, param.uCamMovingRight, param.vCamMovingRight, 'gray')) ./ 255;
